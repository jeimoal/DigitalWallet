const nWallet = {
  ...
  currType: "pt-BR", //BR: "pt-BR", USA: "en", GERMANY: "de-DE", JAPAN: "ja-JP"
  currSymbol: "BRL", //BR:"BRL", USA:"USD", GERMANY:"EUR", JAPAN:"JPY"
  ...
  maskCurrency: (value) => {
    return value.toLocaleString(nWallet.currType, {
      style: "currency",
      currency: nWallet.currSymbol,
    });
  },
  ...
  newWallet: () => {
    ...
    valorTxt.textContent = nWallet.maskCurrency(nWallet.blc);
    ...
    rsvdValue.textContent = nWallet.maskCurrency(nWallet.rsvd);
    ...
    addBtn.addEventListener("click", function () {
      ...
      if (Number.isFinite(vl) && Math.sign(vl) != -1 && vl <= nWallet.limit) {
        ...
        alert(`Value exceded the account limit for deposit. Limit: ${nWallet.maskCurrency(nWallet.limit)}`);
      }
      valorTxt.textContent = nWallet.maskCurrency(nWallet.blc);
    });
    ...
    trashIcon.addEventListener("click", function () {
      if (nWallet.blc != 0) {
        alert(`You need to empty the account first. Available: ${nWallet.maskCurrency(nWallet.blc)}`);
      } else {
